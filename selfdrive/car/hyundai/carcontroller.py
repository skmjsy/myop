        ...
        if 0 < CS.lead_distance <= 149:
          ...
          if self.stopsign_enabled:
            if self.sm['longitudinalPlan'].longitudinalPlanSource == LongitudinalPlanSource.stop:
              ...
              if stop_distance < 50 and not CS.out.cruiseState.standstill:
                apply_accel = self.accel - (DT_CTRL * interp(CS.out.vEgo*CV.MS_TO_MPH, [0.5, 25.0, 40.0, 50.0], [0.5, 5.0, 7.0, 8.0]))
