    ...
    if self.e2e_standstill_enable:
      try:
        ...
        if self.e2e_standstill:
          ...
          self.log.add('self.e2e_standstill')
          if self.e2e_standstill_timer > 100:
            self.e2e_standstill = False
            self.e2e_standstill_timer = 0
            self.log.add('self.e2e_standstill_timer > 100')
        elif CS.clu_Vanz > 0:
          self.e2e_standstill = False
          self.e2e_standstill_stat = False
          self.e2e_standstill_timer = 0
          self.log.add('CS.clu_Vanz > 0')
        ...
        elif self.e2e_standstill_stat and self.sm['longitudinalPlan'].trafficState != 1 and self.sm['longitudinalPlan'].stopLine[12] < 10 and CS.clu_Vanz == 0:
          self.e2e_standstill = True
          self.e2e_standstill_stat = False
          self.e2e_standstill_timer = 0
          self.log.add('self.e2e_standstill = True')
            ...
            if self.sm['longitudinalPlan'].onStop:
              ...
              if 0 <= stop_distance <= 100.0 and not CS.out.cruiseState.standstill:
                ...
                if 0 < stop_distance <= 8.0 and not self.decel_zone3: #force to stop
                  ...
                  if (apply_accel < 0.):
                    #accel = apply_accel * interp(CS.out.vEgo*CV.MS_TO_MPH, [5.0, 10.0, 15.0, 20.0], [1.5, 5.5, 6.0, 6.0]) #test
                    accel = apply_accel * interp(CS.out.vEgo*CV.MS_TO_MPH, [5.0, 10.0], [1.5, 3.0]) #test
                    apply_accel = min(apply_accel, accel)   
                  else:
