...
from selfdrive.ntune import ntune_common_get, ntune_common_enabled, ntune_scc_get, ntune_lqr_get
...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    # hyundai
      ...
      ###############################################################################################
      ret.lateralTuning.lqr.scale = max(ntune_lqr_get('scale'), 1200)
      ret.lateralTuning.lqr.ki = max(ntune_lqr_get('ki'), 0.01)
      ret.lateralTuning.lqr.dcGain = max(ntune_lqr_get('dcGain'), 0.002)
      ret.steerRatio = max(ntune_common_get('steerRatio'), 16.5)
      ret.steerActuatorDelay = max(ntune_common_get('steerActuatorDelay'), 0.3)
      ret.steerLimitTimer = max(ntune_lqr_get('steerLimitTimer'), 2.0)
      ret.steerRateCost = max(ntune_common_get('steerRateCost'), 0.4)
      ret.steerMaxBP = [0.]
      ret.steerMaxV = [max(ntune_lqr_get('steerMax'), 2.0)]
      ###############################################################################################
      ...
      # ret.steerLimitTimer =  2.5
      # ret.steerRateCost = 0.5540000200271606
      # ret.steerMaxBP = [0.]
      # ret.steerMaxV = [2.25]
      ...
      #ret.lateralTuning.indi.outerLoopGainV = [1.8]
