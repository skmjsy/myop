...
HomeWindow::HomeWindow(QWidget* parent) : QWidget(parent) {
  ...
  const char* disable_rotate = "LD_LIBRARY_PATH="" content insert --uri content://settings/system --bind name:s:accelerometer_rotation --bind value:i:0";
  std::system(disable_rotate);
...
void HomeWindow::offroadTransition(bool offroad) {
  ...
  if (offroad) {
    const char* rotate_default = "LD_LIBRARY_PATH="" content insert --uri content://settings/system --bind name:s:user_rotation --bind value:i:1"; //Rotate Comma Landscape:
    std::system(rotate_default);
    slayout->setCurrentWidget(home);
  } else {
    if (Params().getBool("EnableStealthEon")) {
      const char* rotate_portrait = "LD_LIBRARY_PATH="" content insert --uri content://settings/system --bind name:s:user_rotation --bind value:i:0"; //Rotate portrait:
      std::system(rotate_portrait);
    }
