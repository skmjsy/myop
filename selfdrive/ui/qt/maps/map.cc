...
void MapWindow::timerUpdate() {
  ...
  const SubMaster &s = *(uiState()->sm);

  auto gps = s["gpsLocationExternal"].getGpsLocationExternal();
...
void MapWindow::initializeGL() {
  ...
  if (MAPBOX_STYLE == "0") { //comma
    m_map->setStyleUrl("mapbox://styles/commaai/ckr64tlwp0azb17nqvr9fj13s"); // comma
  } else if (MAPBOX_STYLE == "1") { //day
    m_map->setStyleUrl("mapbox://styles/snsincs/ckwgypk5v05uv14mdkir2h5j1"); // day
  } else if (MAPBOX_STYLE == "2") {
    m_map->setStyleUrl("mapbox://styles/snsincs/ckwgywzhv0pxt14na1omjczae"); // night
  } else {
    m_map->setStyleUrl("mapbox://styles/commaai/ckr64tlwp0azb17nqvr9fj13s"); // comma
  }
...
MapInstructions::MapInstructions(QWidget * parent) : QWidget(parent) {
  ...
  main_layout->setContentsMargins(11, 30, 11, 11);
  {
