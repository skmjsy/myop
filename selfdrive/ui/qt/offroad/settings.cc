...
TogglesPanel::TogglesPanel(SettingsWindow *parent) : ListWidget(parent) {
  ...
  std::vector<std::tuple<QString, QString, QString, QString>> toggles{
...
#endif
    {
      ...
      "The screen will automatically switch to Portrait mode.",
      "../assets/offroad/icon_shell.png",
    }, 
...
BrightnessOffControl::BrightnessOffControl() : AbstractControl("Brightness at SCR Off(%)", "When using the EON screen off function, the brightness is reduced according to the automatic brightness ratio.", "../assets/offroad/icon_shell.png") {
  ...
  btnminus.setFixedSize(140, 100);
  btnplus.setFixedSize(140, 100);
...
OPKRMapboxStyle::OPKRMapboxStyle() : AbstractControl("Mapbox Style", "Set the Mapbox sytle to Comma/Day/Night", "../assets/offroad/icon_shell.png") {
...
OPKRServerAPI::OPKRServerAPI() : AbstractControl("User's API", "Set Your API server URL or IP", "") {
  ...
  QObject::connect(&btn, &QPushButton::clicked, [=]() {
    if (btn.text() == "SET") {
      ...
      if (ConfirmationDialog::confirm("Do you want to unset? Device will be rebooted now.", this)) {
...
CommunityPanel::CommunityPanel(QWidget* parent) : QWidget(parent) {
  ...
  toggles.append(new ParamControl("PutPrebuiltOn",
                                            "Use Smart Prebuilt",
                                            "Create a Prebuilt file and speed up booting. When this function is turned on, the booting speed is accelerated using the cache, and if you press the update button in the menu after modifying the code, or if you rebooted with the 'gi' command in the command window, remove it automatically and compile it.",
                                            "../assets/offroad/icon_shell.png",
                                            this));
  ...
  //                                           "Some cars have known radar tracks(from comma) for long control. This uses radar track directly instead of scc can message. Before you go, you must need to run hyundai_enable_radar_points.py in /data/openpilot/selfdrive/debug dir to enable your radar track. (Reboot required)",
  //                                           "../assets/offroad/icon_road.png",
  //                                           this));
  ...
  toggles.append(new ParamControl("MapboxEnabled",
                                            "Enable Mapbox",
                                            "If you want to use Mapbox, turn on and then connect to device using web browser http://<device ip>:8082  Mapbox setting will show up and type mapbox pk and sk token(you can created this on mapbox.com website). If you want to search destinations with googlemap, first, you should create google api key and enable Enable GoogleMap for Mapbox",
                                            "../assets/offroad/icon_shell.png",
                                            this)); 

  toggles.append(new ParamControl("GoogleMapEnabled",
                                            "Use GoogleMap for Mapbox",
                                            "Use GoogleMap when you search a destination.",
                                            "../assets/offroad/icon_shell.png",
                                            this)); 

  toggles.append(new ParamControl("AutoScreenOff",
                                            "Auto-Brightness",
  ...
  toggles.append(new ParamControl("UploadRaw",
                                            "Upload Raw Logs",
                                            "Upload full logs and full resolution video by default while on Wi-Fi.",
                                            "../assets/offroad/icon_shell.png",
                                            this));           

  toggles.append(new ParamControl("OpkrDrivingRecord",
                                            "Use Auto Screen Record",
                                            "Automatically record/stop the screen while driving. Recording begins after departure, and recording ends when the vehicle stops.",
                                            "../assets/offroad/icon_shell.png",
                                            this));
  ...
  toggleLayout->addWidget(horizontal_line());
  toggleLayout->addWidget(new RecordCount());
  toggleLayout->addWidget(horizontal_line());
  toggleLayout->addWidget(new RecordQuality());
  toggleLayout->addWidget(horizontal_line());
  toggleLayout->addWidget(new BrightnessOffControl());
  toggleLayout->addWidget(horizontal_line());
  toggleLayout->addWidget(new AutoShutdown());
  toggleLayout->addWidget(horizontal_line());
  toggleLayout->addWidget(new ForceShutdown());
  toggleLayout->addWidget(horizontal_line());
  toggleLayout->addWidget(new OPKRMapboxStyle());
  toggleLayout->addWidget(horizontal_line());
  toggleLayout->addWidget(new LateralControl());
  toggleLayout->addWidget(horizontal_line());
  ...
  toggleLayout->addWidget(horizontal_line());
  toggleLayout->addWidget(recorddelbtn);
  ...
  toggleLayout->addWidget(horizontal_line());
  toggleLayout->addWidget(realdatadelbtn);
}
