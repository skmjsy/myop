...
static void update_state(UIState *s) {
  ...
  //opkr
   if (!scene.read_params_once) {
     ...
     scene.autoScreenOff = std::stoi(params.get("AutoScreenOff"));
     scene.read_params_once = true;
   }

  // scene.recording_quality = 0; // 1: 960x540, 2: 1280x720
  // scene.recording_count = 200;
  // scene.driving_record = true;
  // scene.brightness_off = 20; //brightness 20%

  if(scene.driving_record)
  {
    dashcam(s);
  } 
}
...
UIState::UIState(QObject *parent) : QObject(parent) {
  sm = std::make_unique<SubMaster, const std::initializer_list<const char *>>({
    ...
    "gpsLocationExternal", "carControl", "liveParameters", "lateralPlan", "longitudinalPlan", "liveMapData"
  });
...
void Device::updateBrightness(const UIState &s) {
  ...
  int appliedBrightness = s.scene.brightness_off;
  
  UIState *ss = uiState();
  ss->scene.getbrightness = brightness;
  ...
  if (!awake) {
    ...
    if (brightness > 98) {
      appliedBrightness = s.scene.brightness_off;
    } else if (brightness <= 98 and brightness > 94) {
      appliedBrightness = s.scene.brightness_off - 10; 
    } else if (brightness <= 94 and brightness > 80) {
      appliedBrightness = s.scene.brightness_off - 20; 
    } else if (brightness <= 80 and brightness > 20) {
      appliedBrightness = s.scene.brightness_off - 30; 
    } else if (brightness <= 20) {
      appliedBrightness = s.scene.brightness_off - 40;
    }
    else {
      appliedBrightness = 10;
    }
    ...
    ss->scene.setbrightness = appliedBrightness;

    if(s.scene.autoScreenOff) {
      brightness = appliedBrightness * 0.01 * brightness;
    } else {
