...
def thermald_thread(end_event, hw_queue):
  ...
  while not end_event.is_set():
    ...
    if should_start:
      ...
      #shutdown battery less 
      if (count % int(1. / DT_TRML)) == 0:
        #print('sec_since_boot', sec_since_boot(), 'off_ts', off_ts, 'usb_power', msg.deviceState.usbOnline)  
        if opkrForceShutdown != 0 and not onroad_conditions["ignition"] and not msg.deviceState.usbOnline:
          opkrForceShutdown = interp(int(params.get("OpkrForceShutdown", encoding="utf8")), [0,1,2,3,4,5], [0,60,180,300,600,1800])
          print('The System will shut down in', abs(int(sec_since_boot() - off_ts - opkrForceShutdown)), 'seconds.'); 
          if (sec_since_boot() - off_ts) > opkrForceShutdown and opkrForceShutdown:
            os.system('LD_LIBRARY_PATH="" svc power shutdown')
          #elif msg.deviceState.batteryPercent < 10:
          #  os.system('LD_LIBRARY_PATH="" svc power shutdown')       
