...
class Controls:
  ...
  def state_transition(self, CS):
    ...
    useSpeedController = False

    slcState = self.sm['longitudinalPlan'].speedLimitControlState
    speed_limit = self.sm['longitudinalPlan'].speedLimit 

    tscState = self.sm['longitudinalPlan'].turnSpeedControlState
    tsc_speed = self.sm['longitudinalPlan'].turnSpeed 
    ...
    #turnSpeedControl
    if tscState > 0 and int(tsc_speed) > 0 and int(tsc_speed) > int(MIN_CURVE_SPEED) and int(tsc_speed) < int(max_speed_limit):
      self.v_cruise_kph = tsc_speed * CV.MS_TO_KPH
      useSpeedController = True
    else:
      tscState = 0
    ...
    if tscState < 1:
      if vtcState == 1:
        ...
        self.PrevSpeed = CS.vEgo
      elif vtcState == 2:
        self.events.add(EventName.visionTurning)
      elif vtcState == 3:
        self.events.add(EventName.visionleaving)
        if int(vtc_speed) >= int(max_speed_limit) and int(self.PrevSpeed) >= int(60 * CV.MPH_TO_MS):
          vtc_speed = max_speed_limit   
        # vtc_speed = float(max(vtc_speed, self.PrevSpeed))
      ...
      if vtcState > 0 and vtcState < 3 and int(vtc_speed) > int(MIN_CURVE_SPEED)  and int(vtc_speed) < int(max_speed_limit): #MIN_CURVE_SPEED = 19mph, MAX_HIGHWAY_SPEED = 65mph
